...
LDAP uses LDAPS with user bind. Type only your sAMAccountName at login; set bind template to `{username}@your.domain` (e.g. `{username}@nexstar.tv`). TLS cert ignore is applied before connect. AD group names can be mapped to roles for automatic access without pre-creating accounts. Add groups under **Admin → LDAP Groups**, and use **Edit** to change roles assigned to an existing group (users get updated roles on their next sign-in).
